...
cleaned_df=df.dropna()#it removes all the rows that contain at least one missing value and creates a new dataframe called "cleaned_df"
print(cleaned_df)
...
print(col)
#inplace attribute is used to modify the original dataframe instead of creating a new one. If inplace=True, the original dataframe will be modified and the method will return None. If inplace=False (the default), a new dataframe will be returned with the modifications, and the original dataframe will remain unchanged.
